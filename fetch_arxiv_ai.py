"""arXiv AI Papers RSS Feed → Notion DB 자동 수집기

arXiv cs.AI 카테고리 RSS 피드를 수집한다.
"""
...
SEEN_FILE = Path(__file__).parent / "seen_arxiv_ai.json"

RSS_FEEDS = [
    {
        "url": "http://export.arxiv.org/rss/cs.AI",
        "source_type": "arXiv AI",
    },
]
...
def fetch_rss(feed_url):
    ...
    headers = {"User-Agent": "Mozilla/5.0 (compatible; ArxivAIBot/1.0)"}
    ...
    # RSS 2.0 (arXiv uses RDF/RSS 1.0 format)
    for item in root.findall(".//{http://purl.org/rss/1.0/}item"):
        title = item.findtext("{http://purl.org/rss/1.0/}title", "").strip()
        link = item.findtext("{http://purl.org/rss/1.0/}link", "").strip()

        # arXiv RSS doesn't have pubDate per item, use today's date
        iso_date = _today.isoformat()

        desc = item.findtext("{http://purl.org/rss/1.0/}description", "").strip()
        desc = re.sub(r'<[^>]+>', '', desc)[:500]

        # Clean up arXiv title (remove "arXiv:XXXX.XXXXX" prefix)
        title = re.sub(r'^.*?\.\s*', '', title, count=1) if title.startswith('arXiv:') else title

        if title:
            items.append({"title": title, "url": link, "date": iso_date, "description": desc})

    # Standard RSS 2.0 fallback
    ...
    for entry in root.findall(".//atom:entry", ns):
        ...
        iso_date = updated[:10] if updated else _today.isoformat()
...
def save_to_notion(items, source_type):
    ...
    # arXiv는 하루에 수백 개의 논문이 올라오므로 최대 5개만 처리
    MAX_ITEMS = 5
