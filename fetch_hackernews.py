"""Hacker News Show HN RSS → Notion DB 자동 수집기

Hacker News의 AI 관련 Show HN, AI 스타트업 뉴스를 수집한다.
"""
...
SEEN_FILE = Path(__file__).parent / "seen_hackernews.json"

RSS_FEEDS = [
    {
        "url": "https://hnrss.org/show?q=AI&points=50",
        "source_type": "Hacker News",
    },
    {
        "url": "https://hnrss.org/newest?q=AI+startup&points=30",
        "source_type": "Hacker News",
    },
]
...
def fetch_rss(feed_url):
    ...
    headers = {"User-Agent": "Mozilla/5.0 (compatible; HackerNewsRSSBot/1.0)"}
...
def save_to_notion(items, source_type):
    ...
    for item in new_items:
        ...
        save_original_subpage(notion, page["id"], item["title"], article_text, item["url"], source_type)
        seen.add(item["url"])
    ...
    for feed in RSS_FEEDS:
        ...
        try:
            ...
            if items:
                ...
                print(f"  수집됨: {len(items)}개")
            else:
                print("  아이템을 찾지 못했습니다.")
        except Exception as e:
            print(f"  ❌ 에러: {e}")

    # 두 피드의 아이템을 합쳐서 한 번에 저장
    if all_items:
        print(f"\n{'='*60}")
        print(f"전체 수집: {len(all_items)}개 아이템")
        print(f"{'='*60}")
        save_to_notion(all_items, "Hacker News")
    else:
        print("\n  수집된 아이템이 없습니다.")
